...
impl Hash for Material {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        state.write_i128((self.density * 10.0) as i128);
        state.write_i128((self.specific_heat * 10.0) as i128);
        state.write_i128((self.thermal_conductivity.0 * 10.0) as i128);
        state.write_i128((self.thermal_conductivity.1 * 10.0) as i128);
        state.write_i128((self.thermal_conductivity.2 * 10.0) as i128);
    }
}
